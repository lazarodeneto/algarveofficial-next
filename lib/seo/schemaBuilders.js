...
function categorySchemaTypes(category) {
  const mapping = {
    ...
    "luxury-experiences": ["TouristAttraction"],
...
export function buildWebsiteSchema(siteUrl = DEFAULT_SITE_URL) {
  return {
    ...
    potentialAction: {
      ...
      target: {
        "@type": "EntryPoint",
        urlTemplate: `${siteUrl}/${DEFAULT_LOCALE}/stay?q={search_term_string}`,
      },
...
export function buildTouristDestinationSchema({
  ...
  return {
    ...
    "@id": `${url}#destination`,
    image,
    touristType: touristType ?? ["Luxury Traveler", "Cultural Tourist", "Golf Enthusiast", "Beach Lover"],
    includesAttraction: {
