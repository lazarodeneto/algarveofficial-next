...
export function CategoriesSection() {
  ...
  const getCategoryCount = (categoryIds: string[]) => {
    ...
    return listings.filter((listing) => listing.category_id && idSet.has(listing.category_id)).length;
  };

  const categoriesWithListings = useMemo(() => {
    return mergedCategories.filter(
      (category) => getCategoryCount(category.memberIds) > 0
    );
  }, [mergedCategories, listings]);
              ...
              return (
                ...
                >
                  ...
                  <Link
                    href={buildLangPath(
                      langPrefix,
                      `/directory?category=${category.slug}`
                    )}
                  ...
                  >
                    ...
                    <span className="relative z-10 text-body-sm font-medium text-primary mt-auto">
                      {getCategoryCount(category.memberIds)}{" "}
                      {t("sections.categories.listings")}
                    </span>
                  </Link>
                </motion.div>
              );
