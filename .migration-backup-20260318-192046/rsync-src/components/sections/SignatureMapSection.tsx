...
const ListingsLeafletMap = dynamic(
  () => import("@/components/map/ListingsLeafletMap").then((module) => module.default),
  { ssr: false }
);
...
export function SignatureMapSection() {
  ...
  const showLoadingState = !hasHydrated || listingsLoading;
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
          <div>
            ...
            <h2 className="text-title font-serif font-medium text-foreground">
              {t("sections.vip.title", { defaultValue: "Signature" })}{" "}
              <span className="text-gradient-gold">Map</span>
            </h2>
            <p className="text-body text-muted-foreground mt-2 max-w-2xl readable">
              {t("sections.vip.subtitle", { defaultValue: "Our most exclusive listings" })} across the Algarve, all in one interactive view.
            </p>
          </div>
        ...
        <div className="rounded-2xl border border-border/70 bg-card/40 p-3 sm:p-4 backdrop-blur-sm">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {DISCOVERY_FILTERS.map((filter) => {
              ...
              const Icon = FILTER_ICONS[filter.key] ?? Sparkles;
