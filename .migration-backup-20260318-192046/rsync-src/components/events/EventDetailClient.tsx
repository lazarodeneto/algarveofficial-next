...
function EventDetailClientInner({
  ...
  const startDateValue = event.start_date ?? event.created_at ?? new Date().toISOString();
  const endDateValue = event.end_date ?? startDateValue;
  const startDate = parseISO(startDateValue);
  const endDate = parseISO(endDateValue);
  const isSingleDay = startDateValue === endDateValue;
            ...
            >
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {relatedEvents.map((relatedEvent) => {
                  const relStartDateValue =
                    relatedEvent.start_date ?? relatedEvent.created_at ?? new Date().toISOString();
                  const relEndDateValue = relatedEvent.end_date ?? relStartDateValue;
                  const relStartDate = parseISO(relStartDateValue);
                  const relEndDate = parseISO(relEndDateValue);
                  const relIsSingleDay = relStartDateValue === relEndDateValue;
