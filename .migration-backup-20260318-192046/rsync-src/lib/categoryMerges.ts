...
export function buildMergedCategoryOptions(categories: CategoryRow[]): MergedCategoryOption[] {
  ...
  for (const canonicalSlug of canonicalSlugs) {
    ...
    const shortDescription = canonicalCategory?.short_description ?? representative.short_description ?? null;
    const imageUrl = canonicalCategory?.image_url ?? representative.image_url ?? null;
