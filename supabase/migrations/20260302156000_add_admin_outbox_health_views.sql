-- Admin-facing observability views for external outbox operations.
-- Uses security_invoker so underlying table RLS remains enforced.

DROP VIEW IF EXISTS public.admin_external_outbox_by_operation;
CREATE VIEW public.admin_external_outbox_by_operation
...
ORDER BY dead_count DESC, retry_count DESC, queued_count DESC, provider, operation;
DROP VIEW IF EXISTS public.admin_external_outbox_health;
CREATE VIEW public.admin_external_outbox_health
WITH (security_invoker = on)
AS
...
GRANT SELECT ON public.admin_external_outbox_by_operation TO authenticated;
GRANT SELECT ON public.admin_external_outbox_health TO authenticated;
