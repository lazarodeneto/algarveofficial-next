-- Refine owner CRM rollups so metrics reflect operational CRM signals.
-- Keeps the existing RPC contract and strengthens the data feeding /admin/crm.
...
BEGIN
  ...
  WITH owner_ids AS (
    ...
    FROM (
      ...
      WHERE ur.role IN ('admin'::public.app_role, 'editor'::public.app_role, 'owner'::public.app_role)
    ...
    WHERE
      ...
      AND (
        ...
        OR (
          ...
          AND LOWER(COALESCE(oru.subscription_status, '')) IN ('active', 'trialing')
          AND COALESCE(oru.subscription_tier, 'unverified') IN ('verified', 'signature')
        )
        OR (
          v_status_filter = 'inactive'
          AND NOT (
            LOWER(COALESCE(oru.subscription_status, '')) IN ('active', 'trialing')
            AND COALESCE(oru.subscription_tier, 'unverified') IN ('verified', 'signature')
          )
        )
        OR (
          ...
          AND (
            ...
            OR COALESCE(oru.has_email_contact, FALSE) = FALSE
  ...
  metrics AS (
    SELECT
      ...
      COUNT(*) FILTER (
        WHERE LOWER(COALESCE(oru.subscription_status, '')) IN ('active', 'trialing')
          AND COALESCE(oru.subscription_tier, 'unverified') IN ('verified', 'signature')
      )::INTEGER AS subscribed_owners,
      COUNT(*) FILTER (
