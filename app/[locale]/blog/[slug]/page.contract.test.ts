...
describe("localized blog post page route contract", () => {
  it("attaches article-related listing links only to eligible blog articles", () => {
    ...
    expect(page).toContain("await getBeachGuideListings(resolvedLocale, post)");
    expect(page).toContain("await getGolfArticleListings(resolvedLocale, post)");
    expect(page).toContain("await getFamilyArticleListings(resolvedLocale, post)");
    ...
    expect(component).toContain('href={l(`/blog?tag=${encodeURIComponent(tag)}`)}');
    expect(component).toContain("<BeachGuideMap");
    expect(component).toContain("<BeachGuideRelatedCards");
