...
const PRIORITY_CODES = new Set(["PT", "US", "GB", "ES", "FR", "DE", "IT", "CH", "BR", "AE", "NL", "BE", "CA", "AU"]);
...
const ALL_COUNTRIES: CountryDialCode[] = [
  ...
  { code: "TC", name: "Turks and Caicos", flag: "🇹🇨", dialCode: "+1649" },
...
/**
 * Returns all countries with priority countries (Portugal, US, UK, etc.) at the top,
 * followed by all remaining countries sorted alphabetically.
 * Fully static — no external API dependency.
 */
export function useCountryDialCodes() {
  const priority = ALL_COUNTRIES.filter((c) => PRIORITY_CODES.has(c.code));
  const rest = ALL_COUNTRIES.filter((c) => !PRIORITY_CODES.has(c.code));

  // Sort priority: Portugal first, then alphabetical
  priority.sort((a, b) => {
