...
import {
  useAdminHeaderMenu,
  useHeaderMenuMutations,
  HeaderMenuItem,
} from "@/hooks/useHeaderMenu";
...
export default function AdminHeaderMenu() {
  const { data: items = [], isLoading } = useAdminHeaderMenu();
  const { createItem, updateItem, deleteItem, reorderItems } = useHeaderMenuMutations();
  
  const [editingItem, setEditingItem] = useState<HeaderMenuItem | null>(null);
  const [isAddDialogOpen, setIsAddDialogOpen] = useState(false);
  const [deleteConfirmItem, setDeleteConfirmItem] = useState<HeaderMenuItem | null>(null);
  ...
  const openEditDialog = (item: HeaderMenuItem) => {
    setEditingItem(item);
  ...
  const handleToggleActive = (item: HeaderMenuItem) => {
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-serif font-normal">Header Menu</h1>
            <p className="text-muted-foreground">
              Manage top navigation menu items
            </p>
          </div>
        </div>
      </m.div>
      ...
      <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Add a new item to the header navigation menu
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Used for i18n translations. Leave empty to use the name directly.
              </p>
            </div>
          </div>
          <DialogFooter>
