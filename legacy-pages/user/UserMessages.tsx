...
export default function UserMessages() {
  ...
  const getDisplayUnreadCount = (thread: UserChatThread) =>
  ...
  const filteredThreads = normalizedQuery.length === 0
    ...
    : threads.filter((thread) => {
        const ownerName = thread.owner?.full_name?.toLowerCase() ?? "";
        const listingName = thread.listing?.name?.toLowerCase() ?? "";
        return ownerName.includes(normalizedQuery) || listingName.includes(normalizedQuery);
      });
  
  const handleSelectThread = (thread: UserChatThread) => {
      ...
      {threads.length === 0 && !isLoading ? (
        <Card className="bg-card border-border">
          <CardContent className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <h3 className="text-lg font-medium mb-2">{t("dashboard.messages.noConversationsYet")}</h3>
              ...
              <div className="h-auto lg:h-[500px] overflow-y-auto">
                {filteredThreads.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-center px-4">
                    ...
                    <p className="text-muted-foreground">{t("dashboard.messages.noMatchingConversations")}</p>
                  </div>
                ) : (
                        ...
                        <div className="min-w-0 flex items-center gap-2">
                          ...
                          <div className="min-w-0">
                            <p className={cn(
                              ...
                              {thread.listing?.name || "Unknown Listing"}
                            </p>
                            <p className="truncate pr-2 text-xs text-muted-foreground">
                              {thread.owner?.full_name || t("dashboard.messages.businessOwner")}
                            </p>
                          </div>
                        </div>
          ...
          <Card className={cn(
            ...
            {selectedThread ? (
              <div className="flex flex-col h-full">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <span className="font-medium truncate">{selectedThread.listing?.name || "Listing"}</span>
                      ...
                      <div className="flex items-center gap-2 text-sm text-muted-foreground mt-1">
                        <span>{t("dashboard.messages.ownerLabel")}: {selectedThread.owner?.full_name || t("dashboard.messages.businessOwner")}</span>
                      </div>
                      {selectedThread.listing && (
                        ...
                        >
                          {t("dashboard.messages.viewListing")}
                ...
                <ScrollArea className="h-auto p-4 lg:flex-1">
                  {messagesLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-8 text-center">
                      ...
                      <p className="text-muted-foreground">{t("dashboard.messages.noMessagesYet")}</p>
                      <p className="text-sm text-muted-foreground mt-1">{t("dashboard.messages.sendToStart")}</p>
                    </div>
                  ) : (
                    <div className="space-y-4">
                      {messages.map((message) => {
                        const isViewer = message.sender_type === "viewer";
                        return (
                          <div key={message.id} className="flex gap-3">
                            <div className={cn(
                              "h-8 w-8 rounded-full flex items-center justify-center flex-shrink-0",
                              isViewer ? "bg-primary/10" : "bg-muted"
                            )}>
                              {isViewer ? (
                            ...
                            <div className="flex-1">
                              <div className="flex items-center gap-2">
                                <span className="font-medium text-sm">
                                  {isViewer ? t("dashboard.messages.you") : (selectedThread.owner?.full_name || t("dashboard.messages.owner"))}
                                </span>
                ...
                {selectedThread.status === "active" && (
                  <div className="p-4 border-t border-border">
                    <div className="flex gap-2">
                      <Textarea
                        placeholder={t("dashboard.messages.typeYourMessage")}
                        value={replyContent}
                ...
                {selectedThread.status !== "active" && (
                  <div className="p-4 border-t border-border text-center text-muted-foreground text-sm">
                    {t("dashboard.messages.conversationClosed", { status: selectedThread.status })}
                  </div>
                )}
              </div>
            ) : (
              <div className="text-center py-12">
                ...
                <p className="text-muted-foreground">{t("dashboard.messages.selectConversation")}</p>
              </div>
            )}
          </Card>
        </div>
